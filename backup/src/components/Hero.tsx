    ...
    <section className="pt-24 pb-12 md:pt-32 md:pb-20 bg-white overflow-hidden">
      <div className="container-custom">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-gray-900 leading-tight text-left lg:text-left">
              Buy <span className="text-primary-500">Crypto</span> Instantly
              With Cash
            </h1>
            <p className="mt-6 text-lg text-gray-600 leading-relaxed text-left lg:text-left">
          ...
          {/* Right Content - Background Video */}
          <motion.div
            initial={{ opacity: 0, x: 50 }}
            animate={{ opacity: 1, x: 0 }}
          ...
          >
            {/* Video Background */}
            <div className="relative w-full h-[500px] rounded-2xl overflow-hidden shadow-2xl group bg-cream">
              <video
                ref={videoRef}
                autoPlay
                muted
              ...
              {/* Play/Pause Button - Similar to Wealthsimple */}
              <button
                onClick={toggleVideo}
                className="absolute bottom-6 right-6 bg-white/90 backdrop-blur-sm rounded-full p-3 shadow-lg hover:bg-white transition-all duration-200 opacity-0 group-hover:opacity-100"
